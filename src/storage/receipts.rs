...
fn to_js(receipt: &Receipt) -> AppResult<JsValue> {
    serde_wasm_bindgen::to_value(receipt).map_err(|e| AppError::Serialization(format!("{:?}", e)))
}
...
pub async fn get_receipts_for_trip(db: &Rexie, trip_id: &str) -> AppResult<Vec<Receipt>> {
    ...
    let key_range = KeyRange::only(&key).map_err(|e| AppError::Database(format!("{:?}", e)))?;
    let items = index
